...
@click.command(
    help="Post a solution-file to Dodona. "
    "The file has to be in your current working directory, "
    "and this only works if there is a selected exercise. "
    "DodonaCLI will give a try at rendering a small part of the feedback-table"
    " so you can continue working from the terminal, this isn't guarenteed to "
    "provide enough info, but it's doing its best."
)
@click.option(
    "-l", "--use-link",
    help="Post your solutionfile to the link at the first line of your "
    "solutionfile. "
...
@click.option(
    "-c", "--check",
    help="Check the file you provided if the syntax is valid for the "
    "programming language associated with the exercise. "
    "Currently supported languages: bash, python, java, " "javascript. "
    "For Java files, it will use javac to compile all *.java files that "
...
def post(file, use_link, check):
    ...
    if not use_link and not config['exercise_id']:
        print(
            ...
            + "No exercise selected! If you want to use a link at the top of "
    ...
    if use_link:
        with open(file, 'r') as solutionfile:
            ...
            if link_index < 0:
                print(
                    '\n' * settings['new_lines_above']
                    + "No valid link found on the first line of your file. "
            ...
            if link.find("/courses/") != -1:
                course_id_index_start = link.find("/courses/")
                course_id_index_start += len("/courses/")
            ...
            exercise_id_index_start = link.find("/activities/")
            exercise_id_index_start += len("/activities/")

            exercise_id_index_stop = link.find("/", exercise_id_index_start)

            exercise_id = link[exercise_id_index_start:exercise_id_index_stop]

            # We can read the content of the file now as the file-pointer is
            # already at the 2nd line
            if hashbang:
    ...
    set_data.post_solution(
        content, connection, headers, course_id, exercise_id, settings
    )
